...
#include "op.h"
#include "libft.h"
#include <stdlib.h> // Check if its ok : for exit()
#include <fcntl.h>	// ???
#include <unistd.h> // ???
#include <limits.h> // ???
...
# define MODULO_MEMORY(x) ((x) % MEM_SIZE < 0 ? MEM_SIZE + (x) % MEM_SIZE : (x) % MEM_SIZE)
...
typedef struct	s_process
{
	...
	int					carry;
	unsigned int		reg[REG_NUMBER]; // TODO ADD REG_SIZE ?????
...
int				get_player_id_for_biggest_nb(t_flags *f, int last_placed, int *value_to_place);
